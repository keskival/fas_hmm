...
for number_of_hidden_states in range(1, NUMBER_OF_HIDDEN_STATES):
    ...
    print(f"Number of hidden states: {number_of_hidden_states}, Correct log prob min,mean,max: {correct_log_prob}, faulty log prob min,mean,max: {faulty_log_prob}, ROC AUC score: {roc_auc_score}, correct hidden state distribution diffs: {correct_distribution_diff}, faulty hidden state distribution diffs: {faulty_distribution_diff}")
